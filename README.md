...
提交者不依赖 Cloudflare Quick Tunnel，也不提交个人电脑上的临时公网地址。若赛事规则只对“提交自托管接口”要求保持 30 天公网可访问，则平台部署模式下由平台负责容器的评测期可用性；最终仍以赛事方提交页面和书面规则为准。
...
本项目不应被描述为“Add/Search 使用 `gpt-4o-mini`”。如果赛事所说的“Add/Search 只能使用 `gpt-4o-mini`”也禁止本地 embedding 模型，则当前方法不满足该条；如果规则仅约束外部 LLM 调用并允许本地 embedding，则当前实现不调用其他外部模型 API。提交前应以赛事方对该条规则的书面解释为准。
...
将 `.env` 中的 `API_KEY` 替换为平台注入或提交页面配置的密钥，然后运行：
...
完整默认配置见 `.env.example` 和 `app/config.py`。
...
- `.env`、数据库、缓存和测试产物不应提交；
- `.dockerignore` 会排除 `.env`、`data`、测试缓存和虚拟环境；
- Docker 镜像固定 Python 3.11 和 Python 依赖版本；
- embedding 模型在镜像构建阶段下载，运行时离线加载；
- `/health` 不返回密钥、模型路径或数据库内容；
- API 日志不主动打印请求正文；
- 平台可通过 Dockerfile 从仓库独立复现服务。
...
- 服务应保持单 worker；
- 当前 Add/Search 使用本地 MPNet embedding，是否满足赛事的 `gpt-4o-mini` 模型条款必须由赛事方确认。
